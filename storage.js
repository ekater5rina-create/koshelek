...
const Snapshots = {
  ...
  /* Самый свежий снимок, в котором вообще есть операции. Смотреть только
     последний нельзя: если после потери данных произошло сохранение,
     последний снимок окажется пустым и заслонит целые. */
  async bestNonEmpty() {
    return (await this.list()).find((s) => s.count > 0) || null;
  },
};
